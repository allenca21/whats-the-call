...
const CHAPTERS = ['All', 1, 2, 3, 4, 5, 6, 7, 8, 9, 'RVI', 'ADV'];
    ...
    try {
      ...
      if (responseCode === InAppPurchases.IAPResponseCode.OK && results.length > 0) {
        ...
        InAppPurchases.setPurchaseListener(async ({ responseCode, results, errorCode }) => {
          ...
          try { await InAppPurchases.disconnectAsync(); } catch (e) {}
        });
      }
    } catch (e) {
  ...
  const handleRestore = async () => {
    ...
    try {
      ...
      InAppPurchases.setPurchaseListener(async ({ responseCode, results }) => {
        if (responseCode === InAppPurchases.IAPResponseCode.OK) {
          const hasPurchase = results.some(p => p.productId === IAP_PRODUCT_ID);
          if (hasPurchase) {
            setIsPro(true);
            await AsyncStorage.setItem('isPro', 'true');
